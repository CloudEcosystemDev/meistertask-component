...
async function endpointCall(url,options) {
...
function setupUrlParamaters(path, body) {
    return Object.keys(body).reduce(
    (previousValue, currentValue) => path = path.replace(":"+ currentValue, body[currentValue]),
    path
  );
}
